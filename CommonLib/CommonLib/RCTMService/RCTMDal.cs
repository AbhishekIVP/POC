...
namespace com.ivp.rad.RCTMService
{
    class RCTMDal
    {
        ...
        /// </summary>
        /// <returns></returns>
        internal DataSet GetAllRunnableJobs()
        {
            lock (objLock)
            {
                ...
                try
                {
                    if (scheduledJobInterval != string.Empty)
                        timeInt = new TimeSpan(0, 0, 0, 0, Convert.ToInt32(scheduledJobInterval) * -3).
                                                                                            Minutes;
                    dsTemp = new DataSet();
        ...
        internal void UpdateNextScheduledTime(RCTMScheduledJobInfo jobInfo, bool resetCon)
        {
            ...
            try
            {
                lock (objLock)
                {
                    ...
                    {
                        SetNextScheduledTime(jobInfo, jobInfo.NextScheduleTime);
                        return;
                    }
                    ...
                    if (jobInfo.NoOfRuns > 0)
                    {
                        ...
                        {
                            res = CheckActualNoOfRuns(jobInfo);
                            if (res)
                                goto Update;
                            else
                            {
                                SetNextScheduledTime(jobInfo, nextScheduledTime);
                                return;
                            }
                        }
                    }
        ...
        /// </summary>
        private void SetNextScheduledTime(RCTMScheduledJobInfo jobInfo, DateTime nextScheduledTime)
        {
            try
            {
                htParams.Clear();
        ...
        /// <param name="jobInfo">The job info.</param>
        /// <returns></returns>
        private bool CheckActualNoOfRuns(RCTMScheduledJobInfo jobInfo)
        {
            DataSet ds = null;
            bool result = false;
            ds = new DataSet();
