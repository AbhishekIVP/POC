...
{
    internal class RBbgGlobalAPI
    {
        #region class member variables
        static IRLogger mLogger = RLogFactory.CreateLogger("RBbgGlobalAPI");
        ...
        public SecuritiesCollection GetSecurities(RBbgSecurityInfo securityInfo)
        {
            ...
            try
            {
                string authentication = RVendorConfigLoader.GetVendorConfig
                                           (RVendorType.Bloomberg, securityInfo.VendorPreferenceId, RVendorConstant.GLOBALAUTHENTICATION);
                session = RBbgUtils.GetGlobalAPISession(authentication, securityInfo);
        ...
        /// </summary>
        private void processBulkField(Element refBulkField, ref Dictionary<string, Dictionary<string, RVendorFieldInfo>> dictResponse, List<int> bulkListId, string securityName)
        {
            int count = 0;
            ...
            for (int bvCtr = 0; bvCtr < numofBulkValues; bvCtr++)
            {
                ...
                for (int beCtr = 0; beCtr < numofBulkElements; beCtr++)
                {
                    ...
                    RVendorFieldInfo vFieldInfo = new RVendorFieldInfo();
