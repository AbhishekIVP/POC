...
function SMExecuteSynchronously(url, method, args) {
    var executor = new Sys.Net.XMLHttpSyncExecutor();
    var request = new Sys.Net.WebRequest();
    if (url.toLowerCase().includes('commonui/'))
        url = url.replace(/CommonUI\//i, "");

    var hostUrl = setPathSM();

    if (hostUrl.toLowerCase().includes('commonui'))
        hostUrl = hostUrl.replace(/CommonUI/i, "");

    request.set_url(hostUrl + '/' + url + '/' + method);
    request.set_httpVerb('POST');
    request.get_headers()['Content-Type'] = 'application/json; charset=utf-8';
    request.set_executor(executor);
    request.set_body(Sys.Serialization.JavaScriptSerializer.serialize(args));
